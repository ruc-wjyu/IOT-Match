...
class Collate:
    ...
    def __call__(self, batch):
        ...
        if args.model_name == 'nezha':
            ...
            enc_source_batch = self.tokenizer(source_batch, max_length=self.max_seq_len, truncation='only_first')
            source_ids = enc_source_batch["input_ids"]
            source_attention_mask = enc_source_batch["attention_mask"]
            enc_target_batch = self.tokenizer(target_batch, max_length=self.max_seq_len, truncation='only_first')
...
class GenerateModel(nn.Module):
    def __init__(self):
        ...
        if args.model_name == 'nezha':
            ...
            self.gpt_model = GPT2Model.from_pretrained(pretrained_gpt2_fold)

        self.linear = nn.Linear(self.configuration.hidden_size, 3).to(device)

    def forward(self, token_ids, token_type_ids):
        hidden_state = None
        seq2seq_predictions = None
        if args.model_name == 'nezha':
            seq2seq_predictions,  hidden_state = self.bert_model(token_ids, token_type_ids)
        elif args.model_name == 'gpt2':
            seq2seq_predictions, hidden_state = self.gpt_model(input_ids=token_ids, token_type_ids=token_type_ids)
...
class AutoSummary(AutoRegressiveDecoder):
    ...
    def generate(self, text, topk=1):
        ...
        output_ids = self.beam_search([token_ids, segment_ids],
                                       topk)  # 基于beam search
        return ''.join(self.model.tokenizer.convert_ids_to_tokens(output_ids))   # skip_special_tokens=True
